...
export function checkAndTriggerAlerts(
    ...
    if (Math.abs(pc1m) > ALERT_THRESHOLDS.priceChange1m && canAlert(contractAddress, 'priceChange1m')) {
        const direction = pc1m > 0 ? '上涨' : '下跌';
        const changeText = `${Math.abs(pc1m).toFixed(1)}%`;
        message = `${symbol} 1分钟${direction}${changeText}`;
        console.log(`[AlertFlow] [PASSED] 规则 "priceChange1m" 满足条件, 準備觸發提醒。`);
        speak(message);
    ...
    if (Math.abs(pc5m) > ALERT_THRESHOLDS.priceChange5m && canAlert(contractAddress, 'priceChange5m')) {
        const direction = pc5m > 0 ? '上涨' : '下跌';
        const changeText = `${Math.abs(pc5m).toFixed(1)}%`;
        message = `${symbol} 5分钟${direction}${changeText}`;
        console.log(`[AlertFlow] [PASSED] 规则 "priceChange5m" 满足条件, 準備觸發提醒。`);
        speak(message);
        onAlert(message, 'price');
    }
}