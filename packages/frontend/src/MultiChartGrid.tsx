...
const MultiChartGrid: Component<MultiChartGridProps> = (props) => {
  // 1. Static refs for the 16 slots
  ...
  const [chartStates, setChartStates] = createSignal<any[]>(Array(16).fill({
    status: 'Initializing',
    legend: null,
    token: null
  }));
  ...
  onMount(() => {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      const controller = new KlineChartController({
        ...
        onStatusChange: (s: string) => updateChartState(i, { status: s }),
        onLegendChange: (l: any) => updateChartState(i, { legend: l }),
        onViewportChange: (from: number, to: number) => props.onViewportChange({ from, to })
      });
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <span class="symbol-title" style={{ 'font-weight': 'bold' }}>{token.symbol}</span>
